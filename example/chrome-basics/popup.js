...
document.getElementById('btn').addEventListener('click', () => {
    if (!gifWindow) {
        ...
        //gifWindow = window.open('whip.gif', 'GifWindow', 'width=300,height=300,top=0,left=0,resizable=no,movable=no,scrollbars=no,toolbar=no,menubar=no,location=no');
        gifWindow = window.open('whip.gif', 'GifWindow', 'width=300,height=300,top=0,left=0,resizable=no');
        gifWindow.blur();
        gitWindow.focus();
        isPlaying = true;
    } else {
